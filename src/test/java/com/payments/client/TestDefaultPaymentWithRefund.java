...
public class TestDefaultPaymentWithRefund extends BaseTest {
	
	private static final Logger LOG = LoggerFactory.getLogger(TestDefaultPaymentWithRefund.class);
	...
	@Test
	public void test3CaptureAndRefund(){
		LOG.info("Test capture twice and refund");
		RestTemplate client = new RestTemplate();
		HttpHeaders headers = createHeaders(merchantAuth.getAccessToken());
		PaymentRequest request = new PaymentRequest();
		request.setAmount(new BigDecimal("2.5"));
		request.setCard(card.getUid());
		
		TransactionResponse firstCapture = client.exchange(PAYMENTS_URL + "/" + payment.getUid() + "/capture", HttpMethod.POST,new HttpEntity<Object>(request,headers),TransactionResponse.class).getBody();
		...
		TransactionResponse secondCapture = client.exchange(PAYMENTS_URL + "/" + payment.getUid() + "/capture", HttpMethod.POST,new HttpEntity<Object>(request,headers),TransactionResponse.class).getBody();
		Assert.assertNotNull(secondCapture);
		Assert.assertNotNull(secondCapture.getUid());
		Assert.assertEquals(TransactionStatus.COMPLETE, secondCapture.getStatus());;
		
		
		PaymentRequest refund = new PaymentRequest();
		refund.setCard(card.getUid());
		refund.setAmount(new BigDecimal("5"));
